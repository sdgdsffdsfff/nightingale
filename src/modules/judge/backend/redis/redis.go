...
type RedisPublisher struct {
	opts   config.RedisPublisherSection
	conns  []*redis.Pool
	buffer chan *config.Event
	stop   chan struct{}
	closed bool
}
...
func NewRedisPublisher(opts config.RedisPublisherSection) (*RedisPublisher, error) {
	if len(opts.Addrs) == 0 {
	...
	redisp := &RedisPublisher{
		...
		buffer: make(chan *config.Event, opts.BufferSize),
	...
	for i := range opts.Addrs {
		rpool := &redis.Pool{
			...
			IdleTimeout: Duration(opts.IdleTimeout),
			Dial: func() (redis.Conn, error) {
				c, err := redis.DialTimeout("tcp",
					opts.Addrs[i],
					Duration(opts.ConnTimeout),
					Duration(opts.ReadTimeout),
					Duration(opts.WriteTimeout))
...
func (rp *RedisPublisher) Publish(event *config.Event) error {
	if rp.closed {
		return nil
	}
	...
	case rp.buffer <- event:
		// do nothing
	case <-time.After(Duration(rp.opts.BufferEnqueueTimeout)):
		// 入队列超时, 直接写入到redis
		err = rp.push(event)
	}
...
func (rp *RedisPublisher) push(event *config.Event) error {
