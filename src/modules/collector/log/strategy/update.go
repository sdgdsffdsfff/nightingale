...
func GetCollects() []*schema.Strategy {
	var stras []*schema.Strategy
	if config.Config.Collector.SyncCollect {
		strasMap := config.Collect.GetLogConfig()
...
func GetCollectFromFile() []*schema.Strategy {
	...
	files, err := file.FilesUnder(config.Config.LogPath)
	if err != nil {
		logger.Error(err)
		return stras
	}
	...
	for _, f := range files {
		...
		b, err := file.ToBytes(config.Config.LogPath + "/" + f)
		if err != nil {
			logger.Warningf("read file name err:%s %v", f, err)
			continue
		}
